...
function GroupD() {
  ...
  var val = [];
  if(nameCounts != null) {
    const nameCountsArray = Object.keys(nameCounts)?.map(name => ({ name, count: nameCounts[name] }));
    val = nameCountsArray;
  }
  
  var dataPoints = val.map(function(item) {
            ...
            <Box m={5}>
              {group?.groupMembers.map((person, i) => {
                return (
                  ...
                  >
                     <AccountCircle /> &nbsp; &nbsp; {person == userDetails?.email ? "You":person}
                  </Grid>
                );
              })}
            </Box>
